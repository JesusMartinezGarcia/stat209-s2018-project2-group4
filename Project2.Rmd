...
------------------------------------------------------------------------
Outline
1. Finish out the gathering code and commit <tonight>
...
>>>>>>> f7f8edf8f56f904f172896379e22d2f2aa2249fc
combined_data<-left_join(GDP_tb,hivprev)
...
graph_function <-function(low_GDP, high_GDP, graph_type, Fsize, title, exclude= "Anatartica")
{HIV_GDPData <- HIV_GDP_data %>% filter(low_GDP< average_GDP & average_GDP < high_GDP & Country != exclude)
...
HIV_GDP_FPlot<- ggplot(HIV_GDPData, aes(x = Year)) +
...
```{r}
graph_function(0, 175, geom_point, 2.5, "HIV % in Low GDP Per Capita Countries")
```
...
```{r}
graph_function(29000, 50000, geom_step, 2.5, "HIV % in High GDP Per Capita Countires", exclude= "Qatar")
```
